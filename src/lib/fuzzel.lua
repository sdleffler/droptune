...
local strlen,chrat,min,asrt,prs,iprs,typ,upack,tblins,tblsrt,strsub,tru,fal,tblcat = string.len,string.byte,math.min,assert,pairs,ipairs,type,unpack,table.insert,table.sort,string.sub,true,false,table.concat
...
fuzzel[HammingDistance] = function(stringa,stringb)
    ...
    asrt(len == strlen(stringb), tblcat({ha," ",di," cannot be calculated on two strings of different lengths:\"",stringa, "\" \"", stringb, "\""}))
    for i = 1,len do
